# Open PineScript Parser

This project is an open-source parser for TradingView's PineScript language, built using the [Chevrotain](https://chevrotain.io/) parsing toolkit. It consumes the token stream generated by the [Open PineScript Lexer](#) and produces a Concrete Syntax Tree (CST).

The primary goal is to create a robust and accurate parser that can be used for advanced language tooling, such as linters, transpilers, static analyzers, and language servers.

## Target Version: PineScript v2

This parser implementation is based on the official **PineScript v2 Parser Grammar**, which is the counterpart to the v2 lexer. This provides a solid and well-documented foundation for the project.

## Implementation Status

This checklist tracks the implementation progress for all rules defined in the v2 parser grammar. The status is generated by comparing the official grammar against the implemented rules in the `OpenPinescriptParser` class.

### Top-Level & Statements
- [x] `tvscript` (entry point, named `script` in the code)
- [x] `stmt` (statement)
- [x] `global_stmt_or_multistmt`
- [x] `global_stmt_or_multistmt2`
- [x] `global_stmt_content`
- [x] `var_def` (variable definition, `a = 1`)
- [x] `var_defs` (destructuring definition, `[a, b] = f()`)
- [x] `var_assign` (variable assignment, `a := 1`)
- [x] `ids_array` (for destructuring)
- [x] `loop_break` (`break` keyword)
- [x] `loop_continue` (`continue` keyword)

### Function Definitions
- [x] `fun_def_stmt` (function definition statement)
- [x] `fun_def_singleline`
- [x] `fun_def_multiline`
- [x] `fun_head` (function signature `(arg1, arg2)`)
- [x] `fun_body_singleline`
- [x] `fun_body_multiline`
- [x] `local_stmt_singleline`
- [x] `local_stmt_singleline2`
- [x] `local_stmts_multiline`
- [x] `local_stmts_multiline2`
- [x] `local_stmt_multiline`
- [x] `local_stmt_content`

### Control Flow
- [x] `if_expr` (if-then and if-then-else expressions)
- [x] `for_expr` (for loops with and without `by` step)
- [x] `stmts_block` (the body of an `if` or `for` statement)

### Expressions (Operator Precedence)
- [x] `arith_expr` (root expression rule)
- [x] `arith_exprs` (array of expressions, `[1, 2, a]`)
- [x] `ternary_expr` (`a ? b : c`)
- [x] `ternary_expr2`
- [x] `or_expr` (`a or b`)
- [x] `and_expr` (`a and b`)
- [x] `eq_expr` (`a == b`, `a != b`)
...
- [x] `add_expr` (`+`, `-`)
- [x] `mult_expr` (`*`, `/`, `%`)
- [ ] `unary_expr` (**Partially Implemented**: Supports `not a` but is missing unary `+a` and `-a`)
- [x] `sqbr_expr` (square bracket access `a[0]`)
- [x] `atom` (literals, identifiers, parenthesized expressions)

### Function Calls
- [x] `fun_call` (`my_func(a, b)`)
- [x] `fun_actual_args` (arguments wrapper)
- [x] `pos_args` (positional arguments)
- [x] `kw_args` (keyword arguments)
- [x] `kw_arg` (`name=value`)

### Literals
- [x] `literal`
- [x] `num_literal` (Integer or Float)
- [x] `other_literal` (String, Bool, or Color)

## Roadmap

- [ ] **Complete `unary_expr` rule:** Add support for unary plus (`+`) and unary minus (`-`) operators to fully match the grammar.
- [ ] **Create an Abstract Syntax Tree (AST) Visitor:** Implement a CST visitor to transform the output into a more abstract and user-friendly AST.
- [ ] **Add Robust Error Handling:** Improve error messages for common syntax mistakes.
- [ ] **Comprehensive Testing:** Write unit and integration tests covering all grammar rules with valid and invalid PineScript v2 code snippets.
...
<details>
<summary><b>Reference: Full ANTLR v3 Grammar for PineScript v2 Parser</b></summary>
